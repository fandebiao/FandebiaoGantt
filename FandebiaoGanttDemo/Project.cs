...
namespace FandebiaoGanttDemo
{
    ...
    /// </summary>
    [Serializable]
    public class ProjectManager : ProjectManager<TaskBase, object>
    {
        public ProjectManager(List<string> citys) : base(citys)
        {
    ...
    public class ProjectManager<T, R> : IProjectManager<T, R>
        ...
        where R : class
    {
        public HashSet<T> _mRegister = new HashSet<T>();
        public List<T> _mRootTasks = new List<T>();
        public Dictionary<T, List<T>> _mMembersOfGroup = new Dictionary<T, List<T>>(); // Map group to list of members
        public Dictionary<T, HashSet<T>> _mDependantsOfPrecedent = new Dictionary<T, HashSet<T>>(); // Map precendent to list of dependents
        public Dictionary<T, HashSet<R>> _mResourcesOfTask = new Dictionary<T, HashSet<R>>(); // Map task to list of resources
        public Dictionary<T, List<T>> _mPartsOfSplitTask = new Dictionary<T, List<T>>(); // Map split task to list of task parts
        public Dictionary<T, T> _mSplitTaskOfPart = new Dictionary<T, T>(); // Map a task part to the original split task
        public Dictionary<T, T> _mGroupOfMember = new Dictionary<T, T>(); // Map member task to parent group task
        public Dictionary<T, int> _mTaskIndices = new Dictionary<T, int>(); // Map the task to its zero-based index order position
        ...
        /// </summary>
        public ProjectManager(List<string> citys)
        {
            Citys = citys;
            Start = new DateTime(2022,1,1);
            Now = DateTime.Now.Subtract(Start);
        }
        ...
        public void Move(T task, int offset)
        {
            if (task != null && _mRegister.Contains(task) && offset != 0)
            {
                ...
                int indexoftask = Citys.IndexOf(task.City);
                if (indexoftask > -1)
                {
                    ...
                    else if (newindexoftask > taskcount)
                    {
                        newindexoftask = taskcount;
                    }
                    ...
                    if (null == null)
                    {
                        ...
                        if (newindexoftask > Citys.Count() - 1) return;

                        _DetachTask(task);
                        task.City = Citys[newindexoftask];
                        _mRootTasks.Insert(newindexoftask, task);
                    }
                    else if (!displacedtask.Equals(task))
                    {
                        ...
                        if (displacedtaskparent == null) // displacedtask is in root
                        {
                            ...
                            // 拖拽 
                            if (indexofdestinationtask > Citys.Count() - 1) return;

                            _DetachTask(task);
                            task.City = Citys[indexofdestinationtask];
        ...
        //----------------------------------------   任务的 属性 操作   ----------------------------------------//
        public void SetId(T task, int id)
        {
            _SetIdHelper(task, id);
        }
        ...
        private void _SetStartHelper(T task, TimeSpan value)
        {
            if (task.Start != value)
            {
                ...
                else // regular task or a split task, which we will treat normally
                {
                    ...
                    //task.StartDateTime = Start.Add(task.Start);
        ...
        private void _SetEndHelper(T task, TimeSpan value)
        {
            if (task.End != value)
            {
                ...
                else // regular task or a split task, which we will treat normally
                {
                    ...
                    // wangfeng 2022
                    task.isModified = true;
                    //task.EndDateTime = Start.Add(task.End);
        ...
        public void _SetIdHelper(T task, int id)
        {
            task.id = id;
        }
